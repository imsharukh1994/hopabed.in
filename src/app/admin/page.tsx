...
import { getAdminStats, getPendingProperties, verifyProperty } from "@/lib/api";
...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="container-page py-12">
      <h1 className="mb-8 text-3xl font-bold text-ink">Admin Dashboard</h1>
      ...
      <h2 className="mb-4 text-xl font-semibold">Properties Pending Verification</h2>
      {properties.length === 0 && !loading ? (
        <div className="rounded-2xl border border-border bg-white p-12 text-center shadow-sm">
          <CheckCircle className="mx-auto mb-4 h-12 w-12 text-green-500" />
          <h2 className="mb-2 text-xl font-semibold">All caught up!</h2>
          <p className="text-muted">There are no properties waiting for verification.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-border bg-canvas">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-ink-soft">Host</th>
                <th className="px-6 py-4 font-semibold text-ink-soft">Location</th>
                <th className="px-6 py-4 font-semibold text-ink-soft">Type</th>
                <th className="px-6 py-4 font-right font-semibold text-ink-soft">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {properties.map(prop => (
                <tr key={prop._id} className="transition hover:bg-canvas/50">
                  <td className="px-6 py-4 font-medium text-ink-soft">{prop.title}</td>
                  <td className="px-6 py-4">{prop.host?.businessName || prop.host?.user?.name || "Unknown"}</td>
                  <td className="px-6 py-4">{prop.locality}, {prop.city}</td>
                  <td className="px-6 py-4 capitalize">{prop.propertyType}</td>
